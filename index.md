Techniques, stratégies et recettes pour construire une __application web moderne__ avec __plusieurs équipes__ qui peuvent __livrer des fonctionnalités de manière indépendante__.

## Qu'est-ce qu'un Micro Frontends?

Le terme __Micro Frontends__ est apparu pour la première fois dans [ThoughtWorks Technology Radar](https://www.thoughtworks.com/radar/techniques/micro-frontends) à la fin de l'année 2016. Il étend les concepts de micro-services au monde du frontend. La tendance actuelle est de créer une application web riche en fonctionnalités et puissante, en bref, une application à page unique, qui repose sur une architecture de micro-services. Au fil du temps, la couche frontend, souvent développée par une équipe distincte, s'agrandit et devient plus difficile à maintenir. C'est ce qu'on appelle un [Frontend Monolith](https://www.youtube.com/watch?v=pU1gXA0rfwc).

L'idée derrière Micro Frontends est de considérer un site web ou une application web comme __une composition de fonctionnalités__ appartenant à des __équipes indépendantes__. Chaque équipe a un __domaine d'activité distinct__ ou une __mission__ qui lui tient à cœur et dans laquelle elle est spécialisée. Une équipe est __multifonctionnel__ et développe ses fonctionnalités de __bout en bout__, de la base de données à l'interface utilisateur.

Cependant, cette idée n'est pas nouvelle. Elle a beaucoup de points communs avec le concept de [Self-contained Systems](http://scs-architecture.org/). Dans le passé, les approches de ce type portaient le nom de [Frontend Integration for Verticalised Systems] (https://dev.otto.de/2014/07/29/scaling-with-microservices-and-vertical-decomposition/). Mais Micro Frontends est clairement un terme plus convivial et moins encombrant.
